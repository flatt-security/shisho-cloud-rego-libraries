...
#   allowed := true
#
#   d := shisho.decision.aws.logmetric.cmk_changes({
#     "allowed": allowed,
#     "subject": subject,
#     "payload": shisho.decision.aws.logmetric.cmk_changes_payload({
...
# METADATA
# title: "decision.api.shisho.dev/v1beta:aws_logmetric_cmk_changes"
# scope: "rule"
# description: |
...
cmk_changes(d) = x {
	...
	x := {
		"header": cmk_changes_header({
			"allowed": d.allowed,
			"subject": d.subject,
			"locator": cmk_changes_locator(d),
			"severity": cmk_changes_severity(d),
		}),
		"payload": d.payload,
	}
}

cmk_changes_severity(d) := shisho.decision.severity_info {
	d.allowed == true
} else := d.severity {
	not is_null(d.severity)
} else := 0

cmk_changes_locator(d) := d.locator {
	not is_null(d.locator)
} else := ""

cmk_changes_kind = "aws_logmetric_cmk_changes"

cmk_changes_header(h) = x {
	x := {
		...
		"annotations": {
			...
			"decision.api.shisho.dev:aws/cis-benchmark/v3.0.0": "4.7",
			"decision.api.shisho.dev:needs-manual-review": "true",
			"decision.api.shisho.dev:ssc/category": "infrastructure",
		},
		"type": shisho.decision.as_decision_type(cmk_changes_allowed(h)),
	}
}

# Force to allow the given decision following resource exception policy
cmk_changes_allowed(h) {
	data.params != null
...
# METADATA
# title: "Entry of decision.api.shisho.dev/v1beta:aws_logmetric_cmk_changes"
# scope: "rule"
# description: |
#   Emits a decision entry describing the detail of a decision decision.api.shisho.dev/v1beta:aws_logmetric_cmk_changes
#

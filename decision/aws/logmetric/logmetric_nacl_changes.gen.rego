...
#   allowed := true
#
#   d := shisho.decision.aws.logmetric.nacl_changes({
#     "allowed": allowed,
#     "subject": subject,
#     "payload": shisho.decision.aws.logmetric.nacl_changes_payload({
...
# METADATA
# title: "decision.api.shisho.dev/v1beta:aws_logmetric_nacl_changes"
# scope: "rule"
# description: |
...
nacl_changes(d) = x {
	...
	x := {
		"header": nacl_changes_header({
			"allowed": d.allowed,
			"subject": d.subject,
			"locator": nacl_changes_locator(d),
			"severity": nacl_changes_severity(d),
		}),
		"payload": d.payload,
	}
}

nacl_changes_severity(d) := shisho.decision.severity_info {
	d.allowed == true
} else := d.severity {
	not is_null(d.severity)
} else := 0

nacl_changes_locator(d) := d.locator {
	not is_null(d.locator)
} else := ""

nacl_changes_kind = "aws_logmetric_nacl_changes"

nacl_changes_header(h) = x {
	x := {
		...
		"annotations": {
			...
			"decision.api.shisho.dev:aws/cis-benchmark/v3.0.0": "4.11",
			"decision.api.shisho.dev:needs-manual-review": "true",
			"decision.api.shisho.dev:ssc/category": "infrastructure",
		},
		"type": shisho.decision.as_decision_type(nacl_changes_allowed(h)),
	}
}

# Force to allow the given decision following resource exception policy
nacl_changes_allowed(h) {
	data.params != null
...
# METADATA
# title: "Entry of decision.api.shisho.dev/v1beta:aws_logmetric_nacl_changes"
# scope: "rule"
# description: |
#   Emits a decision entry describing the detail of a decision decision.api.shisho.dev/v1beta:aws_logmetric_nacl_changes
#

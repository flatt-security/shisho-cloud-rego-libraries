...
#   allowed := true
#
#   d := shisho.decision.aws.logmetric.network_gateway_changes({
#     "allowed": allowed,
#     "subject": subject,
#     "payload": shisho.decision.aws.logmetric.network_gateway_changes_payload({
...
# METADATA
# title: "decision.api.shisho.dev/v1beta:aws_logmetric_network_gateway_changes"
# scope: "rule"
# description: |
...
network_gateway_changes(d) = x {
	...
	x := {
		"header": network_gateway_changes_header({
			"allowed": d.allowed,
			"subject": d.subject,
			"locator": network_gateway_changes_locator(d),
			"severity": network_gateway_changes_severity(d),
		}),
		"payload": d.payload,
	}
}

network_gateway_changes_severity(d) := shisho.decision.severity_info {
	d.allowed == true
} else := d.severity {
	not is_null(d.severity)
} else := 0

network_gateway_changes_locator(d) := d.locator {
	not is_null(d.locator)
} else := ""

network_gateway_changes_kind = "aws_logmetric_network_gateway_changes"

network_gateway_changes_header(h) = x {
	x := {
		...
		"annotations": {
			...
			"decision.api.shisho.dev:aws/cis-benchmark/v3.0.0": "4.12",
			"decision.api.shisho.dev:needs-manual-review": "true",
			"decision.api.shisho.dev:ssc/category": "infrastructure",
		},
		"type": shisho.decision.as_decision_type(network_gateway_changes_allowed(h)),
	}
}

# Force to allow the given decision following resource exception policy
network_gateway_changes_allowed(h) {
	data.params != null
...
# METADATA
# title: "Entry of decision.api.shisho.dev/v1beta:aws_logmetric_network_gateway_changes"
# scope: "rule"
# description: |
#   Emits a decision entry describing the detail of a decision decision.api.shisho.dev/v1beta:aws_logmetric_network_gateway_changes
#

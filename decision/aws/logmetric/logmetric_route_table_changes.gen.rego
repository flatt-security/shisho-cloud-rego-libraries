...
#     "allowed": allowed,
#     "subject": subject,
#     "payload": shisho.decision.aws.logmetric.route_table_changes_payload({
...
# METADATA
# title: "decision.api.shisho.dev/v1beta:aws_logmetric_route_table_changes"
# scope: "rule"
# description: |
...
route_table_changes(d) = x {
	...
	x := {
		"header": route_table_changes_header({
			"allowed": d.allowed,
			"subject": d.subject,
			"locator": route_table_changes_locator(d),
			"severity": route_table_changes_severity(d),
		}),
		"payload": d.payload,
	}
}

route_table_changes_severity(d) := shisho.decision.severity_info {
	d.allowed == true
} else := d.severity {
	not is_null(d.severity)
} else := 0

route_table_changes_locator(d) := d.locator {
	not is_null(d.locator)
} else := ""

route_table_changes_kind = "aws_logmetric_route_table_changes"

route_table_changes_header(h) = x {
	x := {
		...
		"type": shisho.decision.as_decision_type(route_table_changes_allowed(h)),
	}
}

# Force to allow the given decision following resource exception policy
route_table_changes_allowed(h) {
	data.params != null
...
# METADATA
# title: "Entry of decision.api.shisho.dev/v1beta:aws_logmetric_route_table_changes"
# scope: "rule"
# description: |
#   Emits a decision entry describing the detail of a decision decision.api.shisho.dev/v1beta:aws_logmetric_route_table_changes
#

...
#   allowed := true
#
#   d := shisho.decision.aws.logmetric.security_group_changes({
#     "allowed": allowed,
#     "subject": subject,
#     "payload": shisho.decision.aws.logmetric.security_group_changes_payload({
...
# METADATA
# title: "decision.api.shisho.dev/v1beta:aws_logmetric_security_group_changes"
# scope: "rule"
# description: |
...
security_group_changes(d) = x {
	...
	x := {
		"header": security_group_changes_header({
			"allowed": d.allowed,
			"subject": d.subject,
			"locator": security_group_changes_locator(d),
			"severity": security_group_changes_severity(d),
		}),
		"payload": d.payload,
	}
}

security_group_changes_severity(d) := shisho.decision.severity_info {
	d.allowed == true
} else := d.severity {
	not is_null(d.severity)
} else := 0

security_group_changes_locator(d) := d.locator {
	not is_null(d.locator)
} else := ""

security_group_changes_kind = "aws_logmetric_security_group_changes"

security_group_changes_header(h) = x {
	x := {
		...
		"annotations": {
			...
			"decision.api.shisho.dev:aws/cis-benchmark/v3.0.0": "4.10",
			"decision.api.shisho.dev:needs-manual-review": "true",
			"decision.api.shisho.dev:ssc/category": "infrastructure",
		},
		"type": shisho.decision.as_decision_type(security_group_changes_allowed(h)),
	}
}

# Force to allow the given decision following resource exception policy
security_group_changes_allowed(h) {
	data.params != null
...
# METADATA
# title: "Entry of decision.api.shisho.dev/v1beta:aws_logmetric_security_group_changes"
# scope: "rule"
# description: |
#   Emits a decision entry describing the detail of a decision decision.api.shisho.dev/v1beta:aws_logmetric_security_group_changes
#

...
# METADATA
# title: "decision.api.shisho.dev/v1beta:googlecloud_iam_service_account_admin_separation"
# scope: "rule"
# description: |
...
service_account_admin_separation(d) = x {
	...
	x := {
		"header": service_account_admin_separation_header({
			"allowed": d.allowed,
			"subject": d.subject,
			"locator": service_account_admin_separation_locator(d),
			"severity": service_account_admin_separation_severity(d),
		}),
		"payload": d.payload,
	}
}

service_account_admin_separation_severity(d) := shisho.decision.severity_info {
	d.allowed == true
} else := d.severity {
	not is_null(d.severity)
} else := 0

service_account_admin_separation_locator(d) := d.locator {
	not is_null(d.locator)
} else := ""

service_account_admin_separation_kind = "googlecloud_iam_service_account_admin_separation"

service_account_admin_separation_header(h) = x {
	x := {
		...
		"type": shisho.decision.as_decision_type(service_account_admin_separation_allowed(h)),
	}
}

# Force to allow the given decision following resource exception policy
service_account_admin_separation_allowed(h) {
	data.params != null
...
# METADATA
# title: "Entry of decision.api.shisho.dev/v1beta:googlecloud_iam_service_account_admin_separation"
# scope: "rule"
# description: |

...
#   allowed := true
#
#   d := shisho.decision.googlecloud.logmetric.audit_config_changes({
#     "allowed": allowed,
#     "subject": subject,
#     "payload": shisho.decision.googlecloud.logmetric.audit_config_changes_payload({
...
# METADATA
# title: "decision.api.shisho.dev/v1beta:googlecloud_logmetric_audit_config_changes"
# scope: "rule"
# description: |
...
audit_config_changes(d) = x {
	...
	x := {
		"header": audit_config_changes_header({
			"allowed": d.allowed,
			"subject": d.subject,
			"locator": audit_config_changes_locator(d),
			"severity": audit_config_changes_severity(d),
		}),
		"payload": d.payload,
	}
}

audit_config_changes_severity(d) := shisho.decision.severity_info {
	d.allowed == true
} else := d.severity {
	not is_null(d.severity)
} else := 0

audit_config_changes_locator(d) := d.locator {
	not is_null(d.locator)
} else := ""

audit_config_changes_kind = "googlecloud_logmetric_audit_config_changes"

audit_config_changes_header(h) = x {
	x := {
		...
		"type": shisho.decision.as_decision_type(audit_config_changes_allowed(h)),
	}
}

# Force to allow the given decision following resource exception policy
audit_config_changes_allowed(h) {
	data.params != null
...
# METADATA
# title: "Entry of decision.api.shisho.dev/v1beta:googlecloud_logmetric_audit_config_changes"
# scope: "rule"
# description: |
#   Emits a decision entry describing the detail of a decision decision.api.shisho.dev/v1beta:googlecloud_logmetric_audit_config_changes
#

...
#   allowed := true
#
#   d := shisho.decision.googlecloud.logmetric.custom_role_changes({
#     "allowed": allowed,
#     "subject": subject,
#     "payload": shisho.decision.googlecloud.logmetric.custom_role_changes_payload({
...
# METADATA
# title: "decision.api.shisho.dev/v1beta:googlecloud_logmetric_custom_role_changes"
# scope: "rule"
# description: |
...
custom_role_changes(d) = x {
	...
	x := {
		"header": custom_role_changes_header({
			"allowed": d.allowed,
			"subject": d.subject,
			"locator": custom_role_changes_locator(d),
			"severity": custom_role_changes_severity(d),
		}),
		"payload": d.payload,
	}
}

custom_role_changes_severity(d) := shisho.decision.severity_info {
	d.allowed == true
} else := d.severity {
	not is_null(d.severity)
} else := 0

custom_role_changes_locator(d) := d.locator {
	not is_null(d.locator)
} else := ""

custom_role_changes_kind = "googlecloud_logmetric_custom_role_changes"

custom_role_changes_header(h) = x {
	x := {
		...
		"type": shisho.decision.as_decision_type(custom_role_changes_allowed(h)),
	}
}

# Force to allow the given decision following resource exception policy
custom_role_changes_allowed(h) {
	data.params != null
...
# METADATA
# title: "Entry of decision.api.shisho.dev/v1beta:googlecloud_logmetric_custom_role_changes"
# scope: "rule"
# description: |
#   Emits a decision entry describing the detail of a decision decision.api.shisho.dev/v1beta:googlecloud_logmetric_custom_role_changes
#

...
#   allowed := true
#
#   d := shisho.decision.googlecloud.logmetric.firewall_rule_changes({
#     "allowed": allowed,
#     "subject": subject,
#     "payload": shisho.decision.googlecloud.logmetric.firewall_rule_changes_payload({
...
# METADATA
# title: "decision.api.shisho.dev/v1beta:googlecloud_logmetric_firewall_rule_changes"
# scope: "rule"
# description: |
...
firewall_rule_changes(d) = x {
	...
	x := {
		"header": firewall_rule_changes_header({
			"allowed": d.allowed,
			"subject": d.subject,
			"locator": firewall_rule_changes_locator(d),
			"severity": firewall_rule_changes_severity(d),
		}),
		"payload": d.payload,
	}
}

firewall_rule_changes_severity(d) := shisho.decision.severity_info {
	d.allowed == true
} else := d.severity {
	not is_null(d.severity)
} else := 0

firewall_rule_changes_locator(d) := d.locator {
	not is_null(d.locator)
} else := ""

firewall_rule_changes_kind = "googlecloud_logmetric_firewall_rule_changes"

firewall_rule_changes_header(h) = x {
	x := {
		...
		"type": shisho.decision.as_decision_type(firewall_rule_changes_allowed(h)),
	}
}

# Force to allow the given decision following resource exception policy
firewall_rule_changes_allowed(h) {
	data.params != null
...
# METADATA
# title: "Entry of decision.api.shisho.dev/v1beta:googlecloud_logmetric_firewall_rule_changes"
# scope: "rule"
# description: |
#   Emits a decision entry describing the detail of a decision decision.api.shisho.dev/v1beta:googlecloud_logmetric_firewall_rule_changes
#

...
#     "allowed": allowed,
#     "subject": subject,
#     "payload": shisho.decision.googlecloud.sql.instance_postgresql_log_error_verbosity_payload({
...
# METADATA
# title: "decision.api.shisho.dev/v1beta:googlecloud_sql_instance_postgresql_log_error_verbosity"
# scope: "rule"
# description: |
...
instance_postgresql_log_error_verbosity(d) = x {
	...
	x := {
		"header": instance_postgresql_log_error_verbosity_header({
			"allowed": d.allowed,
			"subject": d.subject,
			"locator": instance_postgresql_log_error_verbosity_locator(d),
			"severity": instance_postgresql_log_error_verbosity_severity(d),
		}),
		"payload": d.payload,
	}
}

instance_postgresql_log_error_verbosity_severity(d) := shisho.decision.severity_info {
	d.allowed == true
} else := d.severity {
	not is_null(d.severity)
} else := 2

instance_postgresql_log_error_verbosity_locator(d) := d.locator {
	not is_null(d.locator)
} else := ""

instance_postgresql_log_error_verbosity_kind = "googlecloud_sql_instance_postgresql_log_error_verbosity"

instance_postgresql_log_error_verbosity_header(h) = x {
	x := {
		...
		"type": shisho.decision.as_decision_type(instance_postgresql_log_error_verbosity_allowed(h)),
	}
}

# Force to allow the given decision following resource exception policy
instance_postgresql_log_error_verbosity_allowed(h) {
	data.params != null
...
# METADATA
# title: "Entry of decision.api.shisho.dev/v1beta:googlecloud_sql_instance_postgresql_log_error_verbosity"
# scope: "rule"
# description: |
#   Emits a decision entry describing the detail of a decision decision.api.shisho.dev/v1beta:googlecloud_sql_instance_postgresql_log_error_verbosity
#

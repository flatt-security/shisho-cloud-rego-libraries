...
#   allowed := true
#
#   d := shisho.decision.googlecloud.sql.instance_postgresql_log_min_messages({
#     "allowed": allowed,
#     "subject": subject,
#     "payload": shisho.decision.googlecloud.sql.instance_postgresql_log_min_messages_payload({
...
# METADATA
# title: "decision.api.shisho.dev/v1beta:googlecloud_sql_instance_postgresql_log_min_messages"
# scope: "rule"
# description: |
...
instance_postgresql_log_min_messages(d) = x {
	...
	x := {
		"header": instance_postgresql_log_min_messages_header({
			"allowed": d.allowed,
			"subject": d.subject,
			"locator": instance_postgresql_log_min_messages_locator(d),
			"severity": instance_postgresql_log_min_messages_severity(d),
		}),
		"payload": d.payload,
	}
}

instance_postgresql_log_min_messages_severity(d) := shisho.decision.severity_info {
	d.allowed == true
} else := d.severity {
	not is_null(d.severity)
} else := 1

instance_postgresql_log_min_messages_locator(d) := d.locator {
	not is_null(d.locator)
} else := ""

instance_postgresql_log_min_messages_kind = "googlecloud_sql_instance_postgresql_log_min_messages"

instance_postgresql_log_min_messages_header(h) = x {
	x := {
		...
		"type": shisho.decision.as_decision_type(instance_postgresql_log_min_messages_allowed(h)),
	}
}

# Force to allow the given decision following resource exception policy
instance_postgresql_log_min_messages_allowed(h) {
	data.params != null
...
# METADATA
# title: "Entry of decision.api.shisho.dev/v1beta:googlecloud_sql_instance_postgresql_log_min_messages"
# scope: "rule"
# description: |
#   Emits a decision entry describing the detail of a decision decision.api.shisho.dev/v1beta:googlecloud_sql_instance_postgresql_log_min_messages
#
